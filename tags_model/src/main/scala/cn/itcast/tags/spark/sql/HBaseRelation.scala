...
class HBaseRelation(context: SQLContext, userSchema: StructType, params: Map[String, String]) extends BaseRelation with TableScan
  ...
  // whereFields字段值的形式：modified[GE]20190601,modified[LE]20191201
  val HBASE_TABLE_WHERE_FIELDS: String = "whereFields"
  ...
  override def buildScan(): RDD[Row] = {
    ...
    //============================================
    //设置过滤条件，格式为modified[GE]20190601,modified[LE]20191201
    val whereFields: String = params.getOrElse(HBASE_TABLE_WHERE_FIELDS,null)
    if (null != whereFields && whereFields.length>0){
      //构建FilterList对象
      val filterList = new FilterList()
      //依据条件语句创建Filter对象
      whereFields.split(SPERATOR).foreach{
        whereField=>
          //解析where cause
          val condition: Condition = Condition.parseCondition(whereField)
          //需要将过滤的字段加入到选取的字段中
